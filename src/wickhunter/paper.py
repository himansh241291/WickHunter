...
class PaperBroker:
    ...
    def __init__(
        ...
        risk_guard: BuyRiskGuard | None = None,
    ...
    def submit_buy(
        ...
        *,
        ...
        if self.position is not None:
            self.audit.append({"time": time, "event": "REJECT", "reason": "position_already_open"})
            return None
        if target <= trigger:
            self.audit.append({"time": time, "event": "REJECT", "reason": "invalid_long_target"})
            return None
        ...
        if not decision.allowed:
            self.audit.append({"time": time, "event": "REJECT", "reason": decision.reason})
            return None
        self.position = PaperPosition(time, entry, stop, target, decision.quantity)
        self.risk_state.trades_today += 1
        self.audit.append({"time": time, "event": "BUY_FILLED", "entry": entry, "quantity": decision.quantity})
    ...
    def _close(self, time: datetime, trigger_price: float, result: str) -> PaperClose:
        ...
        self.audit.append({"time": time, "event": "POSITION_CLOSED", "result": result, "exit": exit_price, "net_pnl": net})
        self.position = None
        return close
