...
def test_prepare_sessions_respects_local_timezone_boundary():
    # 23:30 UTC belongs to the next local calendar day in Asia/Kolkata.
    candles = [
        Candle(datetime(2026, 1, 2, 23, 29, tzinfo=timezone.utc), 100, 102, 99, 101),
        Candle(datetime(2026, 1, 2, 23, 30, tzinfo=timezone.utc), 101, 103, 98, 102),
        Candle(datetime(2026, 1, 3, 23, 30, tzinfo=timezone.utc), 102, 106, 101, 105),
    ]
    sessions, levels = prepare_sessions(candles, "Asia/Kolkata")
    assert list(sessions) == ["2026-01-03", "2026-01-04"]
    assert levels["2026-01-04"].pdh == 103
    assert levels["2026-01-04"].pdl == 98
...
def test_load_m1_csv_rejects_duplicate_timestamp(tmp_path):
    path = tmp_path / "data.csv"
    path.write_text(
        "time,open,high,low,close\n"
        "2026-01-02T09:00:00+00:00,100,101,99,100\n"
        "2026-01-02T09:00:00+00:00,100,102,98,101\n",
        encoding="utf-8",
    )
