...
def test_replay_does_not_fill_on_intent_timestamp_itself():
    ticks = [
        Tick(datetime(2026, 1, 2, 9, 1, tzinfo=timezone.utc), 101),
        Tick(datetime(2026, 1, 2, 9, 1, 30, tzinfo=timezone.utc), 100),
    ]
    state = replay_buy_intents(
        ticks,
        [{
            "time": "2026-01-02T09:01:00+00:00",
            "trigger": 101,
            "stop": 99,
            "target": 105,
...
def test_replay_uses_explicit_session_timezone_for_day_boundary():
    ...
    intents = [
        ...
        {"time": "2026-01-02T18:31:00+00:00", "trigger": 102, "stop": 100, "target": 106, "expires_at": "2026-01-02T18:33:00+00:00"},
    ]
